...
**What works is not what we first thought, and it is free.** Size-conditioned prompting (whose reported gain turns out to be an artifact of the binarizer, vanishing under every calibrated rule), multi-scale ensembling, scale-matched retraining (which a noise probe shows learned resize-interpolation shortcuts, ANOVA p≈4×10⁻²⁵¹) and a uniformity regularizer all fail to beat the plain baseline. Evaluating the frozen model at a smaller query window appears to help, but a factorial varying window size and stride independently — which the original sweep never did, having set stride = window/2 at every point — reassigns most of that gain to **sampling density rather than receptive field** (+0.076 against +0.020). The largest effect in the project lies somewhere else entirely. The sliding window gives every voxel it covers the same scalar score, so the heatmap is piecewise-constant over stride³ blocks; weighting each window's contribution by a centre-peaked kernel instead — same model, same windows, **no retraining and no additional forward passes** — collapses the tied plateau from 4,096 voxels to 1, takes small enhancing tumor from 0 of 21 to 7 of 21, and is worth **+0.145 Dice** at the original protocol: more than any window or threshold change, at 71× less compute than the best densely-sampled alternative. Recomputing the window decomposition under that read-out removes the receptive-field term altogether, so the intervention four earlier sections were refining was largely compensating for the rule used to measure it.
...
**A better operating point than Section 7.11 recommended.** Of the eight cells above, **16³/stride 4** is the strongest: it beats the recommended 12³/6 by +0.0238 top-1% (p=1.5×10⁻¹⁵) and +0.0297 oracle. Filling in the gaps below moves the optimum slightly again, to 14³. But either costs roughly 70× the baseline's forward passes, and Section 7.14 shows a larger gain is available for none.
...
| 16³/4 uniform (best cell in the factorial) | 24,389 | 0.4376 |
| 14³/4 uniform (best uniform cell tested) | 24,389 | 0.4405 |
| **32³/16 gaussian** | **343** | **0.4532** |
| 16³/4 gaussian | 24,389 | 0.4874 |

The centre-weighted read-out at the *original* window and stride beats **every uniform condition tested**, including the best one, while doing **71× fewer forward passes** — indeed, exactly the forward passes the published protocol was already paying for. Combining it with denser sampling adds more (0.4874), but the first and largest step costs nothing.
...
- **Test whether the anisotropy result is the whole story** — *built and queued, but not completed in time.* RQ7 found that random vectors matching PubMedBERT's geometry are as good as PubMedBERT, while orthonormal vectors are reliably worse. That points at the geometry rather than the meaning, but the design cannot actually separate them: every condition carrying the geometry also carried the semantics, and every condition stripped of semantics also lost the geometry, so the claim rests on a diagonal through a 2×2 with two empty cells.

    The missing cell is implemented in this repository. `build_rq7_text_variants.py` adds a `whitened` condition: applying **W = G^(−1/2)** to the four class embeddings drives their Gram matrix to the identity, removing the anisotropy — verified at a maximum off-diagonal cosine of 2.2×10⁻⁸ after the transform, against PubMedBERT's +0.9910. Because W is invertible, the rows still span the same subspace and each remains a linear combination of the originals, so the semantic content is untouched and only the angles between classes change. `whitened` is registered in `train_rq7.py` and `evaluate_rq7.py` alongside the original four conditions and runs through the identical path.

    The three seeded training runs were submitted but never reached the front of the cluster queue before this report was due, so **no whitened result is claimed here**. The prediction is stated in advance rather than fitted afterwards: if whitened performs like `randvec_ortho`, which shares its geometry and has no semantics at all, RQ7's conclusion is confirmed by the control it was missing; if it performs like PubMedBERT, the semantics were doing work RQ7's design could not see and Section 7.8's headline needs qualifying. Running it costs three training runs and three evaluations, roughly 1.5 GPU-hours.
