...
RQ3b only compared one alternative (16³) against the 32³ baseline. To see whether the trend continues, reverses, or plateaus, we extended the same isolated-window evaluation (no ensembling, no retraining, same frozen RQ1 model) to two smaller sizes: 12³ (stride 6) and 8³ (stride 8; a non-overlapping tiling rather than the 50%-overlap convention used elsewhere, for compute reasons — flagged here rather than left implicit).

![Mean Dice (pooled across size bins) vs. sliding-window size, one line per region. The trend from RQ3b continues smoothly through 12³ and 8³ with no sign of reversal yet.](figures/fig5_window_curve.png){width=80%}

The trend **continues in the same direction at every step**, with no plateau or reversal through 8³:

| Region | 32³ (baseline) | 16³ | 12³ | 8³ |
|---|---|---|---|---|
| ET large | 0.149 | 0.160 | 0.161 | 0.177 |
| TC large | 0.176 | 0.198 | 0.208 | 0.242 |
| WT large | 0.281 | 0.343 | 0.360 | 0.426 |

More importantly, re-running the full BH-FDR correction across all 72 tests now accumulated across the project (rather than just the 54 from before RQ3c), **ET's statistical picture actually improves as the window shrinks further**: at 12³, 2 of ET's 3 bins now survive correction (only ET-large is borderline, q=0.052); at 8³, **all 3 of ET's bins survive**, along with every other region and bin (18/18 at this window size and the one below it). The region that couldn't clear a corrected significance bar at 16³ becomes the region with the cleanest signal at 8³. This is a genuinely useful result for Future Work: it suggests the receptive-field mismatch identified back in RQ2 was, if anything, understated, and the next natural experiment is finding where (if anywhere) this trend actually stops.
...
- **Only 3 splits checked, not full k-fold.** RQ1's core monotonic pattern was confirmed on two additional independent random splits (Section 6), and held in all 3 (9/9 region×bin replications), which substantially reduces the risk this is an artifact of one lucky/unlucky split. A full k-fold sweep, and re-running RQ2 through RQ5 across multiple seeds rather than just RQ1, would still be a stronger confirmation.
...
- **Find where the window-size trend actually stops.** RQ3c pushed the isolated-window evaluation down to 8³ and the improvement was still growing with no plateau. The natural next step is even smaller windows (4³, 2³) to find the point of diminishing or reversing returns, and to check whether that point is consistent across regions.
