"""RQ2 evaluation: ensemble over small/medium/large text-query heatmaps (true size unknown at eval time),
then re-run the same size-stratified Dice/IoU analysis as RQ1 for direct comparison."""
import argparse
import csv
import os
import random
...
from evaluate_rq1 import SIZE_CUTOFFS, dice_iou, otsu_threshold, size_bin
...
CKPT_PATH = "/net/projects/ranalab/rajhansini/nlp_project/checkpoints/rq2_size_conditioned_aligner_best.pt"
...
def main():
    """Evaluate RQ2 with the size-phrasing ensemble (max over small/medium/large text queries)."""
    ...
    ckpt_path = CKPT_PATH if args.seed == 0 else os.path.join(
        os.path.dirname(CKPT_PATH), f"rq2_size_conditioned_seed{args.seed}_aligner_best.pt")
    print("checkpoint:", ckpt_path)
    model.load_state_dict(torch.load(ckpt_path, map_location=device, weights_only=True))
    model.eval()
    with torch.no_grad():
        text_proj = model.encode_text(text_embeds.to(device))  # (10, proj_dim)
    ...
    for region in ["ET", "TC", "WT"]:
        size_idxs = [class_to_idx[f"{region}_{s}"] for s in ["small", "medium", "large"]]
        for i, pid in enumerate(val_ids):
            ...
            # ensemble: query with all three size-phrasings, take the max response per voxel
            # (mirrors real deployment, where the true size is exactly what we don't know yet)
            heatmaps = [
                sliding_window_heatmap(model, image, text_proj[idx], patch_size=32, stride=16, device=device).numpy()
    ...
    csv_path = os.path.join(RESULTS_DIR, "rq2_localization_scores.csv" if args.seed == 0
                            else f"rq2_seed{args.seed}_localization_scores.csv")
    ...
    print("\n=== RQ2 Summary: mean Dice by region x size bin (size-conditioned ensemble) ===")
    for region in ["ET", "TC", "WT"]:
