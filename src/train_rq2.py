"""RQ2 mitigation: train with size-conditioned text targets (10-way: {ET,TC,WT}x{small,medium,large} + NONE)."""
import argparse
import os
import random
...
from dataset_rq2 import BraTSSizeConditionedPatchDataset
...
def main():
    ...
    train_ds = BraTSSizeConditionedPatchDataset(PREPROCESSED_DIR, LESION_CSV, train_ids, patch_size=args.patch_size)
    val_ds = BraTSSizeConditionedPatchDataset(PREPROCESSED_DIR, LESION_CSV, val_ids, patch_size=args.patch_size)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(CKPT_DIR, "rq2_size_conditioned_aligner_best.pt"))

    torch.save(model.state_dict(), os.path.join(CKPT_DIR, "rq2_size_conditioned_aligner_last.pt"))
