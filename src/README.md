...
| File | LOC | Purpose |
|---|---|---|
| `train_baseline.py` | 89 | P′ baseline / RQ1: 4-way (ET/TC/WT/NONE) contrastive classification. |
...
| File | LOC | Purpose |
|---|---|---|
| `evaluate_rq1.py` | 126 | Core size-stratified Dice/IoU evaluation. Defines `otsu_threshold`, `dice_iou`, `size_bin` used throughout the rest of the project. |
...
| `make_figure4_scale_comparison.py` | 62 | Figure 4: RQ1 vs. RQ3b vs. RQ4 grouped bar chart. |
| `generate_example_heatmaps.py` | 50 | Recomputes and saves the two example heatmaps used by `make_overlay_figure.py`. |

**Total: 2,173 lines across 24 files.**
