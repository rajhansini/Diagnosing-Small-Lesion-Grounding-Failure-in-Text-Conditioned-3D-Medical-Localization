...
**Totals: 62 files, 11,471 lines.** Line counts below are `wc -l` including docstrings and comments.
...
| `build_rq7_text_variants.py` | 204 | Builds the five RQ7 text-encoder ablation conditions: BERT-base, random-init BERT, random orthonormal vectors, random vectors resampled to PubMedBERT's anisotropic geometry, and `whitened` — PubMedBERT's real embeddings with the anisotropy removed by an invertible W = G^(-1/2) map, which keeps the semantics and discards only the geometry. That last condition is RQ7's missing cell; it is built and registered but its training runs did not clear the cluster queue in time, so no result from it is reported (Section 12). |
