...
**One intervention works, and it is the simplest.** Size-conditioned prompting (whose reported gain turns out to be a binarizer artifact, vanishing under every calibrated rule), multi-scale ensembling, scale-matched retraining (which a noise probe shows learned resize-interpolation shortcuts, ANOVA p≈4×10⁻²⁵¹) and a uniformity regularizer all fail to beat the plain baseline. What does work is evaluating the frozen model at a smaller query window — around 12³–16³ instead of 32³, no retraining: +0.060 Dice under a deployable top-1% threshold and +0.067 under an oracle, 2–3x more than the Otsu protocol that discovered it could show. It also lifts threshold-free pointing accuracy for enhancing tumor in all three size bins, taking the previously-at-chance small-ET bin from 0 of 21 patients to 4 of 21.
